...
static int _mvefile_open(MVEFILE *movie, int stream);
...
static int _mvestream_open(MVESTREAM *movie, int stream);
 ...
 * open an MVE file
 */
MVEFILE *mvefile_open(int stream)
{
    MVEFILE *file;
 ...
 * play next chunk
 */
extern const char *mve_major_names[];
int mve_play_next_chunk(MVESTREAM *movie)
{
    ...
    while (major != 0xff)
    {
        ...
        if (major < 32  &&  movie->handlers[major] != NULL)
        {
            ...
            //printf("opcode %02x:%02x size %d %s\n", major, minor, len, mve_major_names[major]);
 ...
 */
static int _mvefile_open(MVEFILE *file, int stream)
{
    file->stream = stream;
    if (file->stream == -1)
        return 0;
