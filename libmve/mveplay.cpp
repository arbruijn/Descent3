...
const char *mve_major_names[] = {
	"MVE_OPCODE_ENDOFSTREAM",
	"MVE_OPCODE_ENDOFCHUNK",
	"MVE_OPCODE_CREATETIMER",
	"MVE_OPCODE_INITAUDIOBUFFERS",
	"MVE_OPCODE_STARTSTOPAUDIO",
	"MVE_OPCODE_INITVIDEOBUFFERS",
	"06",
	"MVE_OPCODE_DISPLAYVIDEO",
	"MVE_OPCODE_AUDIOFRAMEDATA",
	"MVE_OPCODE_AUDIOFRAMESILENCE",
	"MVE_OPCODE_INITVIDEOMODE",
	"0B",
	"MVE_OPCODE_SETPALETTE",
	"MVE_OPCODE_SETPALETTECOMPRESSED",
	"0E",
	"MVE_OPCODE_SETDECODINGMAP",
	"10",
	"MVE_OPCODE_VIDEODATA",
	"12",
	"13",
	"14",
	"15"
...
static void *g_vBackBufSwap;

static int display_video_handler(unsigned char major, unsigned char minor, unsigned char *data, int len, void *context)
{
	void *buf = g_vBackBuf1;
	//unsigned short *dbuf = (unsigned short *)((char *)g_vBackBuf1 + g_width * g_height * 4);
	
	if (g_truecolor) {
		if (!g_vBackBufSwap)
			g_vBackBufSwap = malloc(g_width * g_height * 2);
		unsigned short *sbuf = (unsigned short *)g_vBackBuf1;
		unsigned short *dbuf = (unsigned short *)g_vBackBufSwap;
		int n = g_width * g_height;

		// swap r - b
		for (; n--; ) {
			unsigned short v = *sbuf++;
			*dbuf++ = ((v & 0x7c00) >> 10) | ((v & 0x3e0) << 1) | ((v & 0x1f) << 11);
		}
		buf = g_vBackBufSwap;
	}
	
	//mve_showframe((unsigned char *)g_vBackBuf1, g_screenWidth, g_screenHeight, g_destX, g_destY, g_width, g_height, g_destX, g_destY, g_truecolor);
	int x = (g_screenWidth - g_width) / 2;
	int y = (g_screenHeight - g_height) / 2;
	mve_showframe((unsigned char *)buf, g_width * (g_truecolor ? 2 : 1), g_height, 0, 0, g_width, g_height, x, y, g_truecolor);
...
int MVE_rmPrepMovie(int src, int x, int y, int track)
{
	int i;
	...
	printf("prep 1: %02x\n", mvefile_get_next_segment_major(mve->movie));
	if (mvefile_get_next_segment_major(mve->movie) == MVE_OPCODE_INITVIDEOMODE)
		mve_play_next_chunk(mve); /* video initialization chunk */
	printf("prep 2: %02x\n", mvefile_get_next_segment_major(mve->movie));
	if (mvefile_get_next_segment_major(mve->movie) == MVE_OPCODE_INITAUDIOBUFFERS)
		mve_play_next_chunk(mve); /* audio initialization chunk */
	printf("prep done: %02x\n", mvefile_get_next_segment_major(mve->movie));
